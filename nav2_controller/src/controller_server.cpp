...
ControllerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  teb_path_sub_ = create_subscription<nav_msgs::msg::Path>(
    "local_plan", rclcpp::QoS(10),
    std::bind(&ControllerServer::tebPathCallback, this, std::placeholders::_1));

#if CAR_TYPE == 2
  fw_chassis_sub_ = create_subscription<yhs_can_interfaces::msg::FwChassisInfoFb>(
    "chassis_info_fb", rclcpp::QoS(10),
    std::bind(&ControllerServer::FwChassisCallback, this, std::placeholders::_1));
#endif
...
void ControllerServer::computeControl()
{
  ...
  try {
    ...
    //接收到新的导航点后，设置标志位
    goal_checkers_[current_goal_checker_]->setNewGoal();
...
//    throw nav2_core::PlannerException("Failed to make progress");
    RCLCPP_WARN_ONCE(
    get_logger(), "Sending speed, the chassis does not respond or rotates in place, this warning is only printed once......");
  }
...
#if CAR_TYPE == 2
    //FW车型轮子归零
    std::lock_guard<std::mutex> lock(fw_chassis_lock_);
    if(fabs(fw_chassis_.front_angle_fb.front_angle_fb_l) > 17){
      if(fabs(twist.x) < 0.05 && fabs(cmd_vel_2d.twist.linear.x) >= 0.02 && cmd_vel_2d.twist.linear.y == 0){
        publishZeroVelocity();
        return;
      }
    }
...
void ControllerServer::tebPathCallback(const nav_msgs::msg::Path::SharedPtr msg)
{
  teb_path_ = *msg;
}

#if CAR_TYPE == 2
void ControllerServer::FwChassisCallback(const yhs_can_interfaces::msg::FwChassisInfoFb::SharedPtr msg)
{
  std::lock_guard<std::mutex> lock(fw_chassis_lock_);
